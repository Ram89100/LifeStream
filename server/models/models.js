...
const bloodBankSchema = new mongoose.Schema({
    ...
    resetPasswordToken: { type: String, default: null },
    resetPasswordExpires: { type: Date, default: null },
    requests: [{ requestId: { type: mongoose.Schema.Types.ObjectId, ref: 'Requests' } }],
    donations: [{ donationId: { type: mongoose.Schema.Types.ObjectId, ref: 'Donations' } }],
    stock: {
        'A+': { type: Number, default: 0 },
        'A-': { type: Number, default: 0 },
        'B+': { type: Number, default: 0 },
...
const campSchema = new mongoose.Schema({
    ...
    endTime: { type: String, required: true },
    donors: [{
        _id: { type: mongoose.Schema.Types.ObjectId, ref: 'Users' }, // Removed `unique: true`
        units: { type: Number, required: true, default: 0 },
        status: { type: Number, enum: [0, 1], default: 0 }
    }]
}, { timestamps: true });
